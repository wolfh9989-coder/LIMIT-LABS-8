...
import { AuthAccessClient } from "@/components/AuthAccessClient";
import { sanitizeAuthRedirectPath } from "@/lib/pre-splash-auth";
...
export default async function AuthAccessPage({
  ...
  const nextTarget = sanitizeAuthRedirectPath(params.next ?? null);
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-6 py-12">
        <section className="w-full max-w-xl overflow-hidden rounded-[30px] border border-cyan-300/20 bg-[linear-gradient(180deg,rgba(4,9,19,0.95),rgba(2,4,11,0.98))] shadow-[0_0_0_1px_rgba(34,211,238,0.08),0_0_60px_rgba(34,211,238,0.12)] backdrop-blur-xl">
          ...
          <div className="relative p-8 sm:p-10">
            <div className="mx-auto mb-7 flex w-full max-w-[220px] justify-center">
              <div className="relative animate-[ll8Float_4.8s_ease-in-out_infinite]">
                <div className="absolute inset-4 rounded-[24px] bg-cyan-300/18 blur-3xl" />
                <div className="absolute inset-0 translate-y-4 rounded-[24px] bg-black/55 blur-2xl" />
                <Image
                  ...
                  height={220}
                  priority
                  className="relative h-auto w-full rounded-[24px] border border-cyan-300/22 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.01))] shadow-[0_18px_40px_rgba(0,0,0,0.58),0_0_34px_rgba(34,211,238,0.22),0_0_72px_rgba(217,70,239,0.12)]"
                />
              </div>
            </div>

            <div className="space-y-3 text-center">
              <p className="text-[11px] uppercase tracking-[0.42em] text-cyan-300/72">Access Login</p>
              ...
              <p className="mx-auto max-w-md text-sm leading-6 text-white/64 sm:text-[15px]">
                Continue to your splash onboarding after secure account access.
              </p>
            </div>

            <AuthAccessClient nextTarget={nextTarget} />

            <div className="mt-6 flex items-center justify-between gap-3 text-[11px] uppercase tracking-[0.18em] text-white/34">
              <span>Pre Splash Login</span>
              <span>Beta Auth Layer</span>
            </div>
          </div>
        </section>
      </div>
